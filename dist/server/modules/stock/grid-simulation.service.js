"use strict";
var GridSimulationService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.GridSimulationService = void 0;
const tslib_1 = require("tslib");
const common_1 = require("@nestjs/common");
let GridSimulationService = GridSimulationService_1 = class GridSimulationService {
    logger = new common_1.Logger(GridSimulationService_1.name);
    simulate(params, klines) {
        this.validateParams(params);
        const { basePrice, upperPercent, lowerPercent, gridSpacingPercent, sharesPerGrid, initialShares, initialPrice, commissionRate, minCommission, stampDutyRate, transferFeeRate, } = params;
        const gridLines = this.calcGeometricGridLines(basePrice, upperPercent, lowerPercent, gridSpacingPercent);
        if (klines.length < 2) {
            return {
                totalReturn: 0,
                totalProfit: 0,
            ...
            if (openGrid > currentGrid) {
                for (let g = currentGrid + 1; g <= openGrid; g++) {
                    ...
                    if (holding >= sharesPerGrid) {
                        ...
                        const fee = this.calcFee(amount, 'sell', commissionRate, minCommission, stampDutyRate, transferFeeRate);
                ...
                for (let g = currentGrid - 1; g >= openGrid; g--) {
                    const price = gridLines[g];
                    const amount = price * sharesPerGrid;
                    const fee = this.calcFee(amount, 'buy', commissionRate, minCommission, stampDutyRate, transferFeeRate);
                    holding += sharesPerGrid;
                    cash -= amount + fee;
                    totalFee += fee;
                    trades.push({
        ...
        if (netInvested > peakInvested)
            peakInvested = netInvested;
        ...
        const equityCurve = this.buildEquityCurve(klines, trades, initialShares, startPrice);
    ...
    validateParams(params) {
        if (!params.startDate || !params.endDate) {
            throw new common_1.BadRequestException('请选择回测时间范围');
        }
        if (params.basePrice <= 0) {
            throw new common_1.BadRequestException('基准价必须大于 0');
        }
        if (params.upperPercent < 0) {
            throw new common_1.BadRequestException('上方涨幅不能为负');
        }
        if (params.lowerPercent < 0) {
            throw new common_1.BadRequestException('下方跌幅不能为负');
        }
        if (params.upperPercent === 0 && params.lowerPercent === 0) {
            throw new common_1.BadRequestException('上方涨幅和下方跌幅不能同时为 0');
        }
        if (params.gridSpacingPercent <= 0) {
            throw new common_1.BadRequestException('网格间距必须大于 0');
        }
        if (params.sharesPerGrid <= 0) {
            throw new common_1.BadRequestException('每格买入股数必须大于 0');
        }
        if (params.initialShares < 0) {
            throw new common_1.BadRequestException('初始持仓不能为负');
        }
        if (params.commissionRate < 0) {
            throw new common_1.BadRequestException('佣金费率不能为负');
        }
        if (params.stampDutyRate < 0) {
            throw new common_1.BadRequestException('印花税率不能为负');
        }
        if (params.transferFeeRate < 0) {
            throw new common_1.BadRequestException('过户费率不能为负');
        }
    }
    calcGeometricGridLines(basePrice, upperPercent, lowerPercent, spacingPercent) {
    ...
    getGridIndex(price, gridLines) {
        if (price <= gridLines[0])
            return 0;
    ...
    calcFee(amount, side, commissionRate, minCommission, stampDutyRate, transferFeeRate) {
    ...
    buildEquityCurve(klines, trades, initialShares, startPrice) {
        const curve = [];
        let holding = initialShares;
        let cash = 0;
...
exports.GridSimulationService = GridSimulationService;
exports.GridSimulationService = GridSimulationService = GridSimulationService_1 = tslib_1.__decorate([
