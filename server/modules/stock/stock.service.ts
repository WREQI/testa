...
import { stockBasic, stockKline } from '../../database/schema';
...
const STOCK_POOL_FALLBACK: Array<{ code: string; name: string }> = [
...
interface FinanceRow {
  ...
  totalAssetTurnover?: number | string;
  inventoryTurnover?: number | string;
  revenueGrowth?: number | string;
  netProfitGrowth?: number | string;
  dupontRoe?: number | string;
  dupontAssetTurnover?: number | string;
  dupontEquityMultiplier?: number | string;
}

const BAOSTOCK_TO_DB: Record<string, Record<string, string>> = {
  profit: {
    roeAvg: 'roe',
...
const FINANCE_TYPES: FinanceType[] = ['profit', 'operation', 'growth', 'dupont'];
...
function mapBaostockItem(
  item: Record<string, any>,
  type: FinanceType,
): FinanceRow {
  const statDate: string = item.statDate || '';
  let reportYear = 0;
  let reportQuarter = 4;
  if (statDate && statDate.length >= 10) {
    reportYear = parseInt(statDate.slice(0, 4), 10);
    const month = parseInt(statDate.slice(5, 7), 10);
    if (month <= 3) reportQuarter = 1;
    else if (month <= 6) reportQuarter = 2;
    else if (month <= 9) reportQuarter = 3;
    else reportQuarter = 4;
  }
  ...
  for (const [from, to] of Object.entries(mapping)) {
    if (item[from] !== undefined && item[from] !== '' && item[from] !== null) {
      (row as any)[to] = toNumber(item[from]);
    }
  }
...
export class StockService {
  ...
  constructor(
    ...
    private readonly dataSource: StockSdkDataService,
    private readonly gridSim: GridSimulationService,
    @Inject(DRIZZLE_DATABASE) private readonly db: PostgresJsDatabase,
  ) {}

  private syncInProgress: Promise<void> | null = null;
  private lastSyncDate: string | null = null;
  private industrySyncInProgress: Promise<void> | null = null;
  private lastIndustrySyncDate: string | null = null;
  ...
  async getHotStocks() {
    try {
      ...
      const ordered: HotStock[] = HOT_STOCKS.map((code) => {
      ...
      if (ordered.length > 0) return { items: ordered };
    } catch (e: any) {
    ...
    const basics = await this.db
      ...
      .from(stockBasic)
      .where(inArray(stockBasic.code, HOT_STOCKS));

    const items: HotStock[] = basics.map((b) => ({
      code: b.code,
      name: b.name,
      type: (b.type || 'stock') as 'stock' | 'index',
      close: 0,
      pctChg: 0,
      change: 0,
    }));
  ...
  async getIndices() {
    try {
      ...
      const rtMap = new Map(rt.items.map((q) => [q.code, q]));
      const ordered: IndexQuote[] = INDEX_CODES.map((code) => {
        const q = rtMap.get(code);
        if (q) {
          return {
            code: q.code,
    ...
    const basics = await this.db
      ...
      .from(stockBasic)
      .where(inArray(stockBasic.code, INDEX_CODES));

    const items: IndexQuote[] = basics.map((b) => ({
      code: b.code,
      name: b.name,
      close: 0,
      change: 0,
    ...
    const ordered = INDEX_CODES.map((code) => items.find((i) => i.code === code))
      .filter((v): v is IndexQuote => v !== undefined);
  ...
  async getKline(
    ...
    const cachedDates = cached.map((row: any) => row.tradeDate);
  ...
  private generateMockIndexKline(code: string, days = 45) {
    const items: Array<Record<string, number | string>> = [];
    let seed = 0;
    ...
    const rows = await this.db
      ...
      .where(
        and(
          ...
          eq(stockKline.adjustFlag, adjust),
        ),
      )
    ...
    if (rows.length < 2) {
      try {
        ...
        const result = await this.dataSource.fetchKline({
          ...
          end_date: fmt(today),
          frequency: 'd',
          adjustflag: String(adjust) as any,
        });
        const items = result?.items || [];
        if (items.length > 0) {
          const mapped = items.map((item: any) => ({
  ...
  private buildQuote(code: string, name: string, rows: any[]) {
    const latest = rows[0];
    const prev = rows[1];
  ...
  async getFinance(code: string, type: string, year?: number) {
    if (!FINANCE_TYPES.includes(type as FinanceType)) {
      return { code, type, items: [] };
    }
    const ftype = type as FinanceType;
    ...
    try {
      for (const y of yearsToFetch) {
        try {
          const result = await this.dataSource.fetchFinance(
            ftype,
            code,
            y,
            4,
          ...
          for (const item of items) {
            ...
            if (!row.reportYear) continue;
            await this.cache.upsertFinance(row as any);
          }
        } catch (err: any) {
          this.logger.warn(
            `Finance fetch failed for ${code}/${ftype}/${y}: ${err.message}`,
          );
        }
      }
  ...
  async simulateGrid(code: string, params: GridSimulateRequest) {
    const start = params.startDate;
    const end = params.endDate;

    const klineData = await this.getKline(code, 'd', 2, start, end);

    const items = klineData.items.map((item: any) => ({
      date: item.date,
  ...
  private serializeKlines(rows: any[]) {
    return rows.map((row: any) => ({
  ...
  private async ensureStockListSynced() {
  ...
  private async ensureIndustrySynced() {
    const today = new Date().toISOString().slice(0, 10);
  ...
  private async importStockList(
    items: Array<{ code: string; code_name: string; tradeStatus?: string }>,
  ) {
    const stocks = items
      .filter((item) => {
    ...
    } catch (industryErr: any) {
      this.logger.warn(
        `Industry data sync failed (non-critical): ${industryErr.message}`,
      );
    }
  }

  private async syncIndustryData() {
    this.logger.log('Syncing industry classification data...');
    const result = await this.dataSource.fetchIndustryList();

    const industryItems = (result?.items || []).filter(
      (item) => item.code && item.industry && item.industry.trim() !== '',
    );
    ...
    if (industryItems.length === 0) {
      ...
      const basics = await this.db
        ...
        .from(stockBasic)
        .where(eq(stockBasic.type, 'stock'))
        .limit(5000);
      const mapped = basics.map((b) => {
        const INDUSTRY_BUILTIN: Array<[RegExp, string]> = [
    ...
    this.logger.log(
      `Updating industry info for ${stocksWithIndustry.length} stocks...`,
    );
    await this.cache.batchUpsertStockBasics(stocksWithIndustry);
    this.logger.log('Industry data sync complete');
  }

  private simplifyIndustry(raw: string): string {
    if (!raw) return '';
  ...
  private async getLatestKlines(codes: string[]) {
    const result = new Map<string, { close: number | null; pctChg: number | null }>();
    for (const code of codes) {
      const rows = await this.db
        ...
        .from(stockKline)
        .where(and(eq(stockKline.code, code), eq(stockKline.period, 'd')))
        .orderBy(desc(stockKline.tradeDate))
        .limit(1);
      if (rows.length > 0) {
        result.set(code, {
  ...
  private calcChange(close: number, pctChg: number): number {
    if (!close || !pctChg) return 0;
