...
function builtinIndustryFromName(name: string): string {
  if (!name) return '';
...
function normalizeCode(code: string): string {
  ...
  if (digits.length === 6) {
    if (/^6/.test(digits) || /^9/.test(digits)) return `sh.${digits}`;
    if (/^(0|3)/.test(digits)) return `sz.${digits}`;
    if (/^8|^4/.test(digits)) return `bj.${digits}`;
    return `sh.${digits}`;
  }
...
function toStockSdkCode(internalCode: string): string {
  const c = internalCode || '';
  const digits = c.replace(/[^0-9]/g, '');
  if (digits.length !== 6) return c;
  if (c.startsWith('sh.')) return `SH${digits}`;
  if (c.startsWith('sz.')) return `SZ${digits}`;
  if (c.startsWith('bj.')) return `BJ${digits}`;
...
@Injectable()
export class StockSdkDataService {
  private readonly logger = new Logger(StockSdkDataService.name);
  private stockSdkInstance: any = null;

  constructor() {}

  private async getSdk() {
    if (this.stockSdkInstance) return this.stockSdkInstance;
    try {
      const mod = await import('stock-sdk');
      const Ctor = (mod as any).StockSDK || (mod as any).default?.StockSDK;
      if (Ctor) {
  ...
  async fetchQuotes(codes: string[]): Promise<{ items: RealtimeQuoteRow[] }> {
    const cleanCodes = (codes || []).filter(Boolean);
    if (cleanCodes.length === 0) return { items: [] };
      ...
      if (sdk) {
        ...
        try {
          if (sdk.__stockApiCompat) {
            const stocks = (sdk.mod as any).stocks;
        ...
        if (quotes && quotes.length > 0) {
          const items = quotes
            ...
            .filter((q) => !!q) as RealtimeQuoteRow[];
    ...
    const fallback = cleanCodes.map((c) => this.mockQuote(c)).filter((q) => !!q) as RealtimeQuoteRow[];
    return { items: fallback };
  }

  private mapSdkQuote(q: any): RealtimeQuoteRow | null {
    if (!q) return null;
  ...
  private mockQuote(code: string): RealtimeQuoteRow | null {
    ...
    try {
      ...
      if (sdk) {
        ...
        try {
          if (sdk.__stockApiCompat) {
            const stocks = (sdk.mod as any).stocks;
            const res = await (stocks.auto || stocks.tencent || stocks.sina).getKlines(
              sdkCode,
              { period, count: Math.min(Math.max(daysBetween, 60), 2000) },
            );
            klines = res || [];
  ...
  private mapSdkKline(k: any, frequency: string, adjustflag: number): KlineRawRow | null {
    if (!k) return null;
    const date =
      k.time || k.date || k.tradeDate || k.trade_date || k.day || '';
    let dateStr = '';
  ...
  private softFallbackKline(
    ...
    const items: KlineRawRow[] = [];
    let seed = 0;
    for (let i = 0; i < code.length; i++) seed += code.charCodeAt(i);
    const basePrice =
      8 + (seed % 240) + ((seed * 13) % 500) / 10;
    let price = basePrice;
  ...
  async fetchAllStockList(): Promise<{ items: StockBasicRow[] }> {
    try {
      const sdk = await this.getSdk();
      if (sdk) {
        let rows: any[] = [];
          ...
          if (sdk.__stockApiCompat) {
            ...
            if (typeof fn.searchStocks === 'function') {
              try {
                ...
                const seen = new Map<string, StockBasicRow>();
                for (const ch of letters.split('')) {
                  try {
                    const res = await fn.searchStocks(ch, { count: 120 });
                    const arr = Array.isArray(res) ? res : res?.items || [];
                    for (const item of arr) this.addToListFromSearch(seen, item);
                  } catch {
                    /* ignore */
                  }
                }
  ...
  private augmentWithHotPool(rows: StockBasicRow[]): StockBasicRow[] {
    const hot = this.builtinStockList();
    const map = new Map<string, StockBasicRow>();
    for (const r of rows) if (r?.code) map.set(r.code, r);
    for (const h of hot) if (!map.has(h.code)) map.set(h.code, h);
    return Array.from(map.values());
  }

  private builtinStockList(): StockBasicRow[] {
  ...
  async fetchIndustryList(): Promise<{ items: StockBasicRow[] }> {
    const all = await this.fetchAllStockList();
    return all;
  }

  async fetchFinance(
    type: 'profit' | 'operation' | 'growth' | 'dupont',
    code: string,
    year: number,
    quarter: number,
  ): Promise<{ items: any[] }> {
    ...
    for (const y of years) {
      for (let q = 1; q <= 4; q++) {
        ...
        const row: Record<string, any> = { code, statDate: dateStr };
